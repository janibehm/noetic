      ...
      <section className={pageSection({ space: "xl" })}>
        <div className={pageContainer({ size: "md" })}>
          <div className={stackY({ gap: "md", align: "start" })}>
            <span className="flex gap-2 text-[0.72rem] font-semibold uppercase leading-normal tracking-[0.18em] text-[var(--gray)]">
            ...
            <h1 className="max-w-[78ch] text-[clamp(2.99rem,1.94rem+6.55vw,6.5rem)] font-bold leading-[1.15] tracking-[-0.02em] text-balance">
              {article.title}
            </h1>
            {article.excerpt ? (
              <p className="max-w-[65ch] text-[clamp(1.2rem,1.05rem+0.72vw,1.6rem)] leading-[1.65] text-[var(--gray)] text-pretty">
                {article.excerpt}
              </p>
            ) : null}
            {article.author ? (
              <span className="inline-flex items-center gap-4">
                ...
                <span>
                  ...
                  {article.author.role ? (
                    <span className="block text-sm leading-normal text-[var(--gray)]">
                      {article.author.role}
                    </span>
                  ) : null}
                </span>
