...
export default function LogoMarqueeBlock({ heading, highlightedText, logos = DEFAULT_LOGOS }: LogoMarqueeBlockProps) {
  const row = [...logos, ...logos];
  const parts = heading && highlightedText ? heading.split(highlightedText) : [heading];

  return (
    <section className="py-[clamp(48px,7vw,96px)]">
      {heading ? (
        <Reveal className="mx-auto mb-12 w-full max-w-[var(--maxw)] px-[var(--pad)] text-center">
          <p className="mx-auto max-w-[24ch] text-[clamp(1.4rem,2.4vw,2rem)] font-semibold leading-[1.12] tracking-[-0.03em] text-[var(--gray)]">
            {highlightedText && parts.length > 1 ? (
              <>
      ...
      <Reveal delay={heading ? 1 : 0} className="relative overflow-hidden [mask:linear-gradient(90deg,transparent,#000_12%,#000_88%,transparent)] [-webkit-mask:linear-gradient(90deg,transparent,#000_12%,#000_88%,transparent)]">
