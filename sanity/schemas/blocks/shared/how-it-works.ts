...
export const howItWorksBlock = defineType({
      ...
      of: [
        {
          ...
          fields: [
            ...
            defineField({ name: "body", type: "text", title: "Body", rows: 3, validation: (r) => r.required() }),
            defineField({
              name: "icon",
              type: "string",
              title: "Icon key",
              description: "Identifier mapped to an SVG in the front-end icon registry (e.g. 'prompt', 'grid', 'export').",
            }),
          ],
  ...
  preview: {
    ...
    prepare: ({ blockName, title, steps }) => ({
      ...
      subtitle: steps?.length ? `${steps.length} step(s)` : "Shared block",
    }),
  },
});